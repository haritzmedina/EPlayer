...
var GCPlayerModel = function GCPlayerModel() {};
...
GCPlayerModel.prototype.loadSavedModel = function () {
  ...
  window.GCPlayer.model.extensions.loadSavedModel();
};
...
GCPlayerModel.prototype.playlist.nextSong = function () {
  ...
  if (typeof this.songs[nextSongIndex] === 'undefined') {
    // Is the last song or empty playlist
    if (this.repeatMode === 0) {
      return null;
    } else {
      if (this.songs.length === 0) {
...
GCPlayerModel.prototype.playlist.previousSong = function () {
  ...
  var previousSongIndex = this.current - 1;
  if (typeof this.songs[previousSongIndex] === 'undefined') {
    // Is the last song or empty playlist
    if (this.repeatMode === 0) {
      return null;
    } else {
...
GCPlayerModel.prototype.extensions.loadSavedModel = function(){
  window.GCPlayer.model.getParams(GCPlayerModel.prototype.storage.extensions, function(result){
    window.GCPlayer.model.extensions.data = result;
  });
};

GCPlayerModel.prototype.extensions.saveExtensionData = function(extensionId, callback){
  console.log('Saved extension data of '+extensionId);
  window.GCPlayer.model.setParams({'extensions': window.GCPlayer.model.extensions.data}, (result) => {
    callback(result);
  }, true);
};
...
var Song = function Song(filename, folder, file, title, artist, album, updatedDate) {
  'use strict';
...
var Folder = function Folder(folderPoint, absolutePath) {
  'use strict';
