(function () {
  ...
  var lastfmInitializedPromise = promise.then(function () {
    ...
    console.log(window.GCPlayer.model.extensions.data);

    if(window.GCPlayer.model.extensions.data['lastfm']){
      var config = window.GCPlayer.model.extensions.data;
      debugger;
      if(config.sessionId){
        console.log('Restored lastFM scrobbler session');
      }
    }
  ...
  function loadInterface(){
    ...
    /*$(lastfmMenuContainer).load("extensions/lastfm/lastfm.html", function () {
      ...
      var configData = window.GCPlayer.model.extensions.data['lastfm'];
      ...
      loginYesButton.addEventListener("click", function () {
        window.LastFM.retrieveWebServiceSession(window.GCPlayer.model.extensions.data.lastfm.token, function (session) {
          ...
          // Save sessionID to extensions storage
          window.GCPlayer.model.extensions.saveExtensionData('lastfm', (result) => {
            console.log(result);
          }, true);
